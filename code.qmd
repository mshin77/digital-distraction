...
reverse_thresholds <- function(data) {
  ...
  coef_labels <- c("intercept (Y>1)", "intercept (Y>2)", "intercept (Y>3)", "intercept (Y>4)")

    if (length(coef_labels) != 4) {
    stop("coef_labels must have exactly 4 elements.")
  }
